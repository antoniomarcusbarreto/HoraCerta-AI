...
- `POST /api/auth/request-password-reset` takes `{ name, email, forceSend? }`. Name alone is guessable/public, so it is **not** treated as sufficient proof of identity: the endpoint compares the submitted name against the Firestore profile's `name` field (not Firebase Auth's `displayName`, which registration never sets) for the account matching that email. If they match (`matched: true`), a 6-digit one-time code is generated, hashed (SHA-256) into `passwordResetCodes/{uid}`, and emailed to that account's **own** registered address — proof of inbox access is the real verification factor, not the name. If they don't match — wrong name, or no such account at all, both handled identically so the endpoint never reveals which emails are registered — nothing is sent unless the caller explicitly passes `forceSend: true`, which instead notifies the single admin (`ADMIN_NOTIFICATION_EMAIL` constant) for manual investigation via the **already-existing** `POST /api/admin/change-user-password` flow (`AdminPanel.tsx`'s password modal), relayed to the user out-of-band (WhatsApp/phone/personal email).
